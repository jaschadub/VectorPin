...
from vectorpin import Signer, Verifier, hash_text, hash_vector  # noqa: E402
...
def main() -> int:
    ...
    print(f"# VectorPin Python microbenchmarks")
    info = _platform_info()
    ...
    results: list[Result] = []
    for fn in (bench_hash_text, bench_hash_vector, bench_sign, bench_verify, bench_verify_signature_only):
    ...
    if args.json is not None:
        ...
        args.json.write_text(
            json.dumps(
                {"platform": info, "iters": args.iters, "warmup": args.warmup, "results": [asdict(r) for r in results]},
                indent=2,
            )
        )
        print(f"\nWrote {args.json}")
